...
class ProfileController extends ChangeNotifier {
  ...
  WorkerProfile _profile = WorkerProfile(userId: _localUserId);
  ...
  /// There is no sign-in yet, so everything on this device belongs to one
  /// notional person. P1-8 replaces this with a real account id.
  static const _localUserId = 'local-user';

  void load() {
    _role = UserRole.fromId(_store.readString(StoreKeys.role));

    final raw = _store.readString(StoreKeys.workerProfile);
    if (raw != null && raw.isNotEmpty) {
      try {
        ...
        _profile = WorkerProfile(userId: _localUserId);
      }
    }
  ...
  Future<void> setRole(UserRole role) async {
    ...
    await _store.writeString(StoreKeys.role, role.id);
  }
  ...
  ///   nothing useful.
  /// - A job **posted on this device** always stays visible. Watching your own
  ///   job vanish the moment you post it reads as a failed save.
